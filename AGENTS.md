...
- **Shared:** `NumericConstraintsTrait` (min, max, multipleOf, etc.); `PipelineType` enum; `PipelineStep` value object (internal pipeline entry); variant enums `IpVersion`, `Base64Variant`, `UuidVariant` for format methods
